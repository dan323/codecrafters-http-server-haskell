...
findAcceptEncoding (AcceptEncodingH [] : _) = Nothing
findAcceptEncoding (AcceptEncodingH ua : _) = findValidScheme ua
  where
    findValidScheme [] = Nothing
    findValidScheme (x:xs) 
      | x == "gzip" = Just x
      | otherwise = findValidScheme xs
